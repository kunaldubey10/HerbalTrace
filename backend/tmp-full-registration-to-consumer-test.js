...
(async () => {
  ...
  try {
    ...
    // 7) Admin resolves/creates batch from THIS newly created collection
    console.log('\n7️⃣ Resolving Batch for THIS collection...');
    let batch = collection.autoBatch;
    if (batch && batch.id) {
      const bRes = await api('GET', `/batches/${batch.id}`, adminToken);
      batch = bRes.data || batch;
    } else {
      try {
        ...
        const batchesRes = await api('GET', '/batches', adminToken);
        const batchesList = batchesRes.data || [];
        batch = batchesList.find((b) => b.species === 'Tulsi') || batchesList[0];
      }
    }
    ...
    console.log('   ✅ Batch identified:', batch?.batch_number || batch?.batchNumber, 'ID:', batch?.id);
    ...
    const qcTest = await api('POST', '/qc/tests', labLogin.token, {
      ...
      lab_name: labLogin.user.fullName || labLogin.user.username,
      test_type: 'PURITY',
      species: batch.species || 'Tulsi',
      sample_quantity: 100,
      sample_unit: 'g',
      priority: 'HIGH',
